...
注意：仅仅将tone=开头的行拷贝即可，其类型的配置语句并不兼容不要拷贝。
...
只有老旧的低端设备才需要人工调高度。
XCMania R属于高端设备依赖GPS定位自动校准海拔。您只需要将仪器带到能看到大片天空的户外稍等即可。GPS定位速度与可见天空相关，从几十秒到十五分钟不等。
注意：每天第一次开机都需要大片天空来校准。

为什么你的手机和苹果手表不需要每天校准？
回忆一下你苹果一天充电几次就知道了。R作为滑翔伞设备一年不飞拿出来都可以用哦～

## 蓝牙找不到了怎么办？

请观察屏幕左上角是否有蓝牙标志：

如果有表明你身边的某个手机/平板已经连接并占有设备。请将您身边十米以内配对过的手机、平板全都关机。直至标志消失。注意：由于安卓XCTrack设计缺陷，必须是关机！关App没用！

问：我是小白，开了一个app连上了想换一个app连怎么搞？

答：将连过的设备全部关机，然后开机一台，开一个app去连。
